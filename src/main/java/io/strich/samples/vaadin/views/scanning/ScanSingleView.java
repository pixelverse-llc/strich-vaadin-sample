...
@Route("scan-single")
@JavaScript(value = "https://cdn.jsdelivr.net/npm/@pixelverse/strichjs-sdk@1.12.1/dist/strich-noesm.js", loadMode = LoadMode.EAGER)
public class ScanSingleView extends VerticalLayout {

    public ScanSingleView() {
        setHeightFull();

        add(new H2("Single Scan"));
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        UI.getCurrent().getPage().executeJs("""
                window.onBarcodeScanned = (data) => {
                    console.debug('Destroying BarcodeReader after single scan');
                    window.barcodeReader.destroy().finally(() => {
                        console.debug('Notifying server');
    ...
    private void onBarcodeScanned(String data) {
        Notification.show("Barcode scanned: " + data);
        UI.getCurrent().navigate("/");
    }
}
